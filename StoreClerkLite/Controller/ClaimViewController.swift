    ...
    func claim(_ code: String!) {
        ...
        AlamofireRequestAndResponse.sharedInstance.claimWithCode(code, completionHandler: { (correlationRefId, licenseCode, amount, error) in
            ...
            if error != nil {
                ...
                if let claimAmountViewController = self.storyboard?.instantiateViewController(withIdentifier: "ClaimAmountViewController") as? ClaimAmountViewController {
                    claimAmountViewController.correlationRefId = correlationRefId
                    claimAmountViewController.claimLicenseCode = licenseCode
                    claimAmountViewController.amount = amount
                    self.navigationController?.pushViewController(claimAmountViewController, animated: true)
                }
            }
        })
    }
...
extension ClaimViewController: QRCodePreviewDelegate {
