...
class GTStorage: NSObject {
    ...
    */
    ...
    func getValue(_ keyName: String, fromStore: String) -> AnyObject! {
        ...
        if let path = checkStore(fromStore)! as String? {
            ...
            if let objectValue = dict?.object(forKey: keyName) as AnyObject? {
                return objectValue
            } else {
                return getLatestValue(keyName, fromStore: fromStore)
            }
        } else {
            return nil
        }
    ...
    func getLatestValue(_ keyName: String, fromStore: String) -> AnyObject! {
        if let path = Bundle.main.path(forResource: fromStore, ofType:"plist") {
            let dict :NSDictionary? = NSDictionary(contentsOfFile: path)
            if let objectValue = dict?.object(forKey: keyName) as AnyObject? {
                writeValue(objectValue, forKey: keyName, toStore: fromStore)
                return objectValue
            }
        }
    ...
    */
    ...
    func writeValue(_ value: AnyObject!, forKey: String, toStore: String) {
        ...
        if let path = checkStore(toStore)! as String? {
            ...
            if let old = dict?.object(forKey: forKey) as AnyObject? {
    ...
    */
    ...
    func checkStore(_ storeName: String) -> String? {
        ...
        let storeURL = _documentsDirectory.appendingPathComponent("\(storeName).plist")
